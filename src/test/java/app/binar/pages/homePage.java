...
public class homePage {
    ...
    public static void comparePrice(WebDriver driver, boolean highToLow) {
        List<WebElement> itemPrices = driver.findElements(By.className("inventory_item_price"));
        boolean isSorted = highToLow ? isPriceHighToLowSorted(itemPrices) : isPriceLowToHighSorted(itemPrices);

        Assert.assertTrue(isSorted, highToLow ? "Items are not sorted by price (high to low)" : "Items are not sorted by price (low to high)");
    }

    private static boolean isPriceHighToLowSorted(List<WebElement> itemPrices) {
    ...
    private static boolean isPriceLowToHighSorted(List<WebElement> itemPrices) {
        for (int i = 0; i < itemPrices.size() - 1; i++) {
            double value1 = extractNumericValue(itemPrices.get(i));
            double value2 = extractNumericValue(itemPrices.get(i + 1));
    ...
    public static void verifyProductListIsExist(WebDriver driver){
